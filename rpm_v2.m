...
tec_tic = input('Timing of tectonic uplif events [years B.P.] ( It must be smaller than Maximum Iteration) ');
%tec_tim = maxit - tec_tic;
tec_tim = tec_tic;
tec_amp = input('Verical displacement amount of tectonic uplif events [m] ( Will be rounded to 1 decimal place ) ');
rsl_tic = input('Timing of sea level flexion point [years B.P.] ( It must be smaller than Maximum Iteration) ');
%rsl_tim = maxit - rsl_tic;
rsl_tim = rsl_tic;
rsl_amp = input('Sea level at flexion point above MSL [m] ( Will be rounded to 1 decimal place ) ');
...
if(isempty(rsl_tim))
    ...
    rate = zeros(1,length(rsl_tim)-1);
    for i=1:length(rsl_tim)-1
        rate(i) = (rsl_amp(i+1)-rsl_amp(i))/(rsl_tim(i+1)-rsl_tim(i));
    end
end
                        ...
                        for h=1:length(height_values)
                            ...
                            for resi=1:length(resi_values)
                                ...
                                while (loop<maxit) & ((ix_max<(x_maxcell-(1/gridsize))) |(ix_posi<(x_maxcell-(1/gridsize)))|(ix_tidal_posi<(x_maxcell-(1/gridsize))))                
                                    ...
                                    if(rsl_flag)
                                        if(~isempty(rsl_tim))
                                            if ( loop < rsl_tim(rsl_con) )
                                                
                                                if( abs(rate(rsl_con-1))>=gridsize )
                                                    mwl=mwl+round(rate(rsl_con-1)/gridsize);
                                                else
                                                    if( amp_sum > gridsize)
                                                        mwl=mwl+round(amp_sum/gridsize);
                                                        amp_sum = 0;
                                                    else
                                                        amp_sum = amp_sum + rate(rsl_con-1);
                                                    end
                                                end
                                    ...
                                    %%%
                                    if(tec_flag)
                                        for i=1:length(tec_tim)
                                            if( loop==tec_tim(i) )
                                                mwl = mwl - tec_amp(i)/gridsize; 
                                                break
                                            end
                                        end
                                    end
